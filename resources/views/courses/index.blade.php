...
<head>
    ...
    <style>
        ...
        .profile-circle {
            width: 36px; /* Slightly increased from 32px */
            height: 36px; /* Slightly increased from 32px */
            background: #2196f3;
            border-radius: 50%;
            display: flex;
            align-items: center;
            ...
            background-size: cover;
            background-position: center;
            margin-top: -10px; /* Adjusted from -10px to better align with bell */
        }
...
<body>
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                ...
                                @if(Auth::user()->is_admin)
                                    ...
                                    <li><a class="dropdown-item" href="{{ route('admin.payment-history') }}">
                                        <i class="fas fa-money-bill me-2"></i>Payments
                                    </a></li>
                                    <li><a class="dropdown-item" href="{{ route('admin.user-course', ['tab' => 'payment-settings']) }}">
                            ...
                            <div class="notification-bell position-relative" 
                                ...
                                >
                                    <div class="notification-header">
                                        <h6>Notifications</h6>
                                        <button @click="markAllAsRead" class="mark-all-read" x-show="unreadCount > 0">
    ...
    <div class="position-relative">
        <div class="semester-pills">
            <div class="semester-pill active">Year 1- First semester</div>
            <div class="semester-pill">Year 1- Second semester</div>
            <div class="semester-pill">Year 2- First semester</div>
        </div>
                ...
                <div class="course-card {{ strtolower($course->name) }}" 
                    ...
                    <div class="course-image" style="background-image: url('{{ $course->image_url }}')" onerror="this.style.backgroundImage = 'linear-gradient(135deg, #f5f7fa 0%, #e9ecef 100%)'">
                        ...
                        <div class="course-overlay">
                            <h3 class="course-title" style="color: {{ $course->color }}">{{ $course->name }}</h3>
                        </div>
                    </div>
    ...
    <script>
        // Enhanced semester pills functionality with precise scrolling
        document.querySelectorAll('.semester-pill').forEach(pill => {
            pill.addEventListener('click', () => {
                ...
                const selectedSemester = pill.textContent.trim();
                const coursesGrid = document.querySelector('.courses-grid');
        ...
        try {
            ...
            function initializePaymentProcess(proceedButton, courseId, csrfToken) {
                ...
                proceedButton.addEventListener('click', function() {
                    ...
                    fetch(`/payments/${courseId}/process-manual`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'Accept': 'application/json',
                    ...
                    .then(data => {
                        ...
                        if (data.success) {
                            // Replace modal content with manual payment details
                            document.querySelector('#paymentModal .modal-content').innerHTML = data.html;
                        } else {
                            // Show error message
